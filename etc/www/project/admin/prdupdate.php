...
if(isset($_POST['update']))
{
	if($_POST['update']=="yes")
	{
		...
		if(is_uploaded_file($_FILES['photo']['tmp_name'])){
			...
			$remove=$db->removeimage($image['image']);
		...
		$sql=("UPDATE products SET pname='$name', detail='$detail', category ='$category', brand='$brand', 
...
$cresult=$db->query("select cid, cname from category");
$bresult=$db->query("select id, name from brand");
?>
<html>
	<head>
		<title>MYSTORE</title>
		...
		<div class="wrapper">
			...
			<div class="rightcontent">
				<?php
					...
					<tr>
					<td>Name:</td>
					<td><input type="text" name="name" value="<?php echo $data['pname']; ?>" /></td>
					</tr>
					<tr>
					<td>Detail:</td>
				...
				while($arrayRow = mysql_fetch_assoc($cresult)) {
						$cat = $arrayRow['cname'];
						$cid = $arrayRow['cid'];
						echo "<option selected ='selected' value='$cid'>$cat</option>\n";
					}
					?>
				</select></td></tr>
					<tr><td>Brand:</td>
				...
				while($arrRow = mysql_fetch_assoc($bresult)) {
						$bnd = $arrRow['name'];
						$bid = $arrRow['id'];
						echo "<option selected ='selected' value='$bid'>$bnd</option>\n";
					}
					?>
				</select></td></tr>
					<td>Price:</td>
